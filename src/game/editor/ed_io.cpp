				...
				for(int l = 0; l < pGItem->m_NumLayers; l++)
				{
					...
					if(pLayerItem->m_Type == LAYERTYPE_TILES)
					{
						...
						else if(pTilemapItem->m_Flags&16)
						{
							...
							pTilesTemp = new CLayerSwitchOld(pTilemapItem->m_Width, pTilemapItem->m_Height);
							MakeSwitchLayerOld(pTilesTemp);
						}
						else
						{
						...
						else if(pTiles->m_Switch)
						{
							...
							mem_copy(((CLayerSwitchOld*)pTilesTemp)->m_pSwitchTile, pSwitchData, pTiles->m_Width*pTiles->m_Height*sizeof(CSwitchTileOld));
							for(int i = 0; i < pTiles->m_Width*pTiles->m_Height; i++)
							{
								if(((CLayerSwitchOld*)pTilesTemp)->m_pSwitchTile[i].m_Type)
								{
									((CLayerSwitch*)pTiles)->m_pSwitchTile[i].m_Type = ((CLayerSwitchOld*)pTilesTemp)->m_pSwitchTile[i].m_Type;
									((CLayerSwitch*)pTiles)->m_pSwitchTile[i].m_Number = ((CLayerSwitchOld*)pTilesTemp)->m_pSwitchTile[i].m_Number;
									((CLayerSwitch*)pTiles)->m_pSwitchTile[i].m_Flags = ((CLayerSwitchOld*)pTilesTemp)->m_pSwitchTile[i].m_Flags;
								}
								else
								{
