...
CLayerSwitchOld::CLayerSwitchOld(int w, int h)
: CLayerTiles(w, h)
{
	m_pTypeName = "Switch";
	m_Switch = 1;

	m_pSwitchTile = new CSwitchTileOld[w*h];
	mem_zero(m_pSwitchTile, w*h*sizeof(CSwitchTileOld));
}

CLayerSwitchOld::~CLayerSwitchOld()
{
	delete[] m_pSwitchTile;
}

void CLayerSwitchOld::Resize(int NewW, int NewH)
{
	// resize switch data
	CSwitchTileOld *pNewSwitchData = new CSwitchTileOld[NewW*NewH];
	mem_zero(pNewSwitchData, NewW*NewH*sizeof(CSwitchTileOld));

	// copy old data
	for(int y = 0; y < min(NewH, m_Height); y++)
		mem_copy(&pNewSwitchData[y*NewW], &m_pSwitchTile[y*m_Width], min(m_Width, NewW)*sizeof(CSwitchTileOld));
...
void CLayerSwitchOld::Shift(int Direction)
{
	CLayerTiles::Shift(Direction);
	...
	case 1:
		{
			// left
			for(int y = 0; y < m_Height; ++y)
				mem_move(&m_pSwitchTile[y*m_Width], &m_pSwitchTile[y*m_Width+1], (m_Width-1)*sizeof(CSwitchTileOld));
		}
		break;
	case 2:
		{
			// right
			for(int y = 0; y < m_Height; ++y)
				mem_move(&m_pSwitchTile[y*m_Width+1], &m_pSwitchTile[y*m_Width], (m_Width-1)*sizeof(CSwitchTileOld));
		}
		break;
	case 4:
		{
			// up
			for(int y = 0; y < m_Height-1; ++y)
				mem_copy(&m_pSwitchTile[y*m_Width], &m_pSwitchTile[(y+1)*m_Width], m_Width*sizeof(CSwitchTileOld));
		}
		break;
	case 8:
		{
...
void CLayerSwitchOld::BrushDraw(CLayer *pBrush, float wx, float wy)
{
	CLayerSwitch *l = (CLayerSwitch *)pBrush;
	int sx = ConvertX(wx);
	int sy = ConvertY(wy);
...
void CLayerSwitchOld::FillSelection(bool Empty, CLayer *pBrush, CUIRect Rect)
{
	if(m_Readonly)
		return;
