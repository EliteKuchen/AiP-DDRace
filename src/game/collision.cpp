	...
	for(int i = 0; i < m_Width*m_Height; i++)
	{
		...
		if(m_pFront)
		{
			...
			if(Index <= TILE_NOEXTRAS)
			{
				...
				// DDRace tiles
				if(Index == TILE_THROUGH || (Index >= TILE_FREEZE && Index <= TILE_UNFREEZE) || (Index >= TILE_SWITCHOPEN && Index<=TILE_BOOST) || (Index >= TILE_BEGIN && Index <= TILE_STOPA) || Index == TILE_CP || Index == TILE_CP_F || (Index >= TILE_OLDLASER && Index <= TILE_NPH) || (Index >= TILE_EHOOK_START && Index <= TILE_EHOOK_END) || (Index >= TILE_DFREEZE && Index <= TILE_DUNFREEZE) || (Index >= TILE_RAINBOW && Index <= TILE_BLOODY) || (Index >= TILE_JMPRESET && Index <= TILE_NOEXTRAS))
...
bool CCollision::TileExists(int Index)
{
	...
	if(m_pTiles[Index].m_Index >= TILE_FREEZE && m_pTiles[Index].m_Index <= TILE_NOEXTRAS)
		return true;
	if(m_pFront && m_pFront[Index].m_Index >= TILE_FREEZE && m_pFront[Index].m_Index  <= TILE_NOEXTRAS)
		return true;
